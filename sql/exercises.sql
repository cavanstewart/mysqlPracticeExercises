...
SELECT payment_id, amount, payment_date FROM payment WHERE DATE_FORMAT(payment_date, '%Y-%m-%d') = '2005-05-27';
...
SELECT * FROM customer WHERE NOT active or RIGHT(last_name, 1) = 'M';
...
SELECT title, description, special_features, length, rental_duration FROM film WHERE special_features IS NOT NULL and rental_duration BETWEEN 5 and 7 ORDER BY length DESC LIMIT 10;
...
SELECT f.title, f.description, f.release_year, l.name as language FROM film f LEFT JOIN language l on f.language_id = l.language_id;
...
SELECT s.first_name, s.last_name, a.address, a.address2, c.city, district, postal_code FROM staff s LEFT JOIN address a on a.address_id = s.address_id LEFT JOIN city c on a.city_id = c.city_id;
